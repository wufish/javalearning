 ...
 * 而`*`表示它前面的字符可以出现任意次（包含`0`次）。 在本题中，匹配是指字符串的所有字符匹配整个模式。例如，字符串`aaa`与模式`a
 * .a`和`ab*ac*a`匹配，但是与`aa.a`和`ab*a`均不匹配。
 * <p>
 * ### 解法
 * 判断模式中第二个字符是否是 `*`：
...
public class Q19_RegularExpressionsMatching {
    ...
    private boolean match(char[] str, int i, int len1,
        ...
        if (j + 1 < len2 && pattern[j + 1] == '*') {
            if (i < len1 && (str[i] == pattern[j] || pattern[j] == '.')) {
                return match(str, i, len1, pattern, j + 2, len2)//0次
    ...
    public static void main(String[] args) {
        Q19_RegularExpressionsMatching test = new Q19_RegularExpressionsMatching();
        boolean match = test.match(new char[]{'a', 'b', 'c'}, new char[]{'a', '*'});
